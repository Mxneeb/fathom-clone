import { signIn, auth } from "@/lib/auth";
import { redirect } from "next/navigation";

export default async function LoginPage() {
  const session = await auth();
  if (session?.user) redirect("/calls");

  return (
    <main className="flex flex-1 flex-col items-center justify-center gap-8 px-4">
      <div className="flex flex-col items-center gap-2">
        <div className="text-2xl font-semibold tracking-tight">Fathom Clone</div>
        <p className="text-sm text-neutral-400">Sign in to see your meetings.</p>
      </div>

      <form
        action={async () => {
          "use server";
          await signIn("google", { redirectTo: "/calls" });
        }}
      >
        <button
          type="submit"
          className="flex items-center gap-3 rounded-lg bg-white px-5 py-3 text-sm font-medium text-neutral-900 shadow hover:bg-neutral-100 transition-colors"
        >
          <GoogleIcon />
          Continue with Google
        </button>
      </form>

      <p className="max-w-sm text-center text-xs text-neutral-500">
        Per the build plan, onboarding is real Google OAuth only — no calendar
        connection, no demographic questionnaire, no desktop-app gate.
      </p>

      {process.env.NODE_ENV !== "production" && (
        <a
          href="/api/dev-login"
          className="rounded border border-dashed border-neutral-700 px-3 py-1.5 text-xs text-neutral-400 hover:border-neutral-500 hover:text-neutral-200"
        >
          Dev bypass — continue as Dev User (local only, disabled in
          production)
        </a>
      )}
    </main>
  );
}

function GoogleIcon() {
  return (
    <svg viewBox="0 0 24 24" className="h-4 w-4" aria-hidden>
      <path
        fill="#4285F4"
        d="M23.52 12.27c0-.85-.08-1.67-.22-2.46H12v4.66h6.47c-.28 1.5-1.13 2.77-2.4 3.62v3h3.88c2.27-2.09 3.57-5.17 3.57-8.82z"
      />
      <path
        fill="#34A853"
        d="M12 24c3.24 0 5.95-1.07 7.94-2.91l-3.88-3c-1.08.72-2.45 1.15-4.06 1.15-3.13 0-5.78-2.11-6.73-4.95H1.27v3.1C3.25 21.3 7.31 24 12 24z"
      />
      <path
        fill="#FBBC05"
        d="M5.27 14.29A7.2 7.2 0 0 1 4.9 12c0-.8.14-1.57.37-2.29v-3.1H1.27A11.98 11.98 0 0 0 0 12c0 1.94.46 3.77 1.27 5.39l4-3.1z"
      />
      <path
        fill="#EA4335"
        d="M12 4.77c1.77 0 3.35.61 4.6 1.8l3.44-3.44C17.94 1.19 15.24 0 12 0 7.31 0 3.25 2.7 1.27 6.61l4 3.1C6.22 6.88 8.87 4.77 12 4.77z"
      />
    </svg>
  );
}
